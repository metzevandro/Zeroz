...
function useInputTime({ value, onChange, }) {
    ...
    const togglePicker = React.useCallback(() => setIsOpen((prev) => !prev), []);
    const closePicker = React.useCallback(() => setIsOpen(false), []);
    const handleHourSelect = React.useCallback((hour) => {
        setSelectedHour(hour);
        onChange(`${hour}:${selectedMinute}`);
    }, [onChange, selectedMinute]);
    const handleMinuteSelect = React.useCallback((minute) => {
        setSelectedMinute(minute);
...
function useInfiniteScroll({ values, selected, onSelect, }) {
    ...
    const getIndexForValue = React.useCallback((val, offset = midOffset) => {
...
function TimeColumn({ values, selected, onSelect }) {
    ...
    const repeatedValues = React.useMemo(() => Array.from({ length: REPEAT_COUNT }, () => values).flat(), [values]);
    const columnHeight = VISIBLE_ITEMS * ITEM_HEIGHT;
    return (jsxRuntime.jsxs("div", { className: "time-column", style: { height: columnHeight, position: "relative" }, children: [jsxRuntime.jsx("div", { className: "time-column-highlight", style: {
                    position: "absolute",
                    top: "50%",
                    left: 0,
                    right: 0,
                    ...
                    pointerEvents: "none",
                    zIndex: 1,
                } }), jsxRuntime.jsxs("div", { ref: scrollRef, className: "time-column-scroll", style: {
                    height: "100%",
                    overflowY: "scroll",
                    scrollbarWidth: "none",
                }, children: [jsxRuntime.jsx("div", { style: { height: Math.floor(VISIBLE_ITEMS / 2) * ITEM_HEIGHT } }), repeatedValues.map((value, index) => (jsxRuntime.jsx("button", { className: `input-time-buttons ${value === selected ? "selected" : ""}`, style: {
                            height: ITEM_HEIGHT,
                            display: "flex",
                            alignItems: "center",
                            justifyContent: "center",
                            width: "100%",
                        }, onClick: () => onSelect(value), "aria-label": value, tabIndex: value === selected ? 0 : -1, children: value }, index))), jsxRuntime.jsx("div", { style: { height: Math.floor(VISIBLE_ITEMS / 2) * ITEM_HEIGHT } })] })] }));
}
...
const InputTime = ({ label, placeholder, disabled = false, error = false, textError, onChange, value, ...rest }) => {
    ...
    return (jsxRuntime.jsxs("div", { className: "input-time", ref: modalRef, children: [jsxRuntime.jsx(Input.Input, { label: label, type: "text", value: displayValue, placeholder: placeholder, icon: "schedule", disabled: disabled, onClick: togglePicker, onChange: handleInputChange, error: error, textError: textError, inputMode: "numeric", pattern: "[0-9]*", ...rest }), isOpen && (jsxRuntime.jsxs("div", { className: "input-time-modal", children: [jsxRuntime.jsx("div", { className: "modal-section", children: jsxRuntime.jsx(TimeColumn, { values: HOURS, selected: selectedHour, onSelect: handleHourSelect }) }), jsxRuntime.jsx("div", { className: "modal-section", children: jsxRuntime.jsx(TimeColumn, { values: MINUTES, selected: selectedMinute, onSelect: handleMinuteSelect }) })] }))] }));
};
