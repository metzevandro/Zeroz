 ...
 */
function SelectDropdown({ options, selected, onSelect, isOpen, }) {
    return (jsx("ul", { className: `input-select-dropdown ${isOpen ? "open" : "close"}`, "aria-hidden": isOpen ? 'true' : 'false', role: "listbox", children: options.map((option) => (jsx("li", { role: "option", "aria-selected": option === selected, children: jsx("button", { className: `input-select-option ${option === selected ? "selected" : ""}`, onClick: () => onSelect(option), type: "button", children: option }) }, option))) }));
}
...
const InputSelect = ({ options, label, error = false, errorMessage, disabled = false, onChange, value, }) => {
    ...
    return (jsxs("div", { className: "input-select-root", ref: dropdownRef, children: [label && (jsx("label", { id: labelId, className: "input-select-label", htmlFor: uid, children: label })), jsxs("div", { className: "input-select", children: [jsxs("button", { id: uid, ref: triggerRef, type: "button", className: getTriggerClass(!!selectedOption, error, disabled), "aria-haspopup": "listbox", "aria-expanded": isOpen, "aria-labelledby": labelId, disabled: disabled || error, onClick: isOpen ? close : open, onKeyDown: handleKeyDown, children: [jsx("span", { className: selectedOption ? "" : "input-select-placeholder", children: selectedOption ?? label ?? "Select an option" }), jsx(Icon, { icon: "keyboard_arrow_down", size: "sm" })] }), jsx(SelectDropdown, { isOpen: isOpen, options: options, selected: selectedOption, onSelect: selectOption })] }), error && errorMessage && (jsx("p", { className: "input-select-error-message", children: errorMessage }))] }));
};
