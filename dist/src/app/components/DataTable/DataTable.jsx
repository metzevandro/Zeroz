...
var DataTable = function (_a) {
    ...
    var renderHeader = function () {
        if (isAnyItemSelected) {
            return (<>
          <Modal footer={<FooterModal>
                <div style={{ width: "min-content" }}>
                  <ButtonGroup contentFirstButton="Delete" contentSecondButton="Cancel" direction="row" variantFirstButton="warning" variantSecondButton="secondary" onClickFirstButton={removeRow} onClickSecondButton={toggleModal}/>
                </div>
              </FooterModal>} description="Are you sure that you want delete this row?" dismissible={true} title="Delete" isOpen={isOpen} hideModal={toggleModal}/>
          <div className="data-table-header-selected-items">
            ...
            <div className="data-table-header-selected-items-buttons">
              <Button size="md" variant="secondary" label="Export" onClick={exportSelectedRowsAsCSV} typeIcon="download"/>
              <Button size="md" variant="secondary" label={selectableLabelSecondButton} onClick={toggleModal} typeIcon={selectableIconSecondButton}/>
            </div>
          </div>
        </>);
        }
        ...
        <InputSearch placeholder={inputPlaceholder} onChange={handleSearchChange}/>
        {filters && (<div className="data-table-header-actions">
            <div style={{ width: "100%" }}>
              <Button variant="secondary" typeIcon={typeIconSecondButton} size="md" label={labelSecondButton} onClick={toggleAside}/>
            </div>
          </div>)}
      </div>);
    };
    ...
    var renderNoDataMessage = function () {
        ...
        <EmptyState icon="search_off" title={titleNoDataMessage} description={descriptionNoDataMessage}/>
      </div>);
    };
    var renderNoDataFilteredMessage = function () { return (<div className="render-no-data-message">
      <EmptyState icon="search_off" title={titleNoDataFilteredMessage} description={descriptionNoDataFilteredMessage} buttonContentPrimary={labelButtonNoDataFilteredMessage} onClickActionPrimary={handleClearFilters}/>
    </div>); };
    ...
    return (<>
      <div className="data-table-root">
          ...
          {filteredData.length > 0 ? (<>
              <div className="data-table-content-header" style={calculateGridTemplate(selectable, expandable)}>
                {expandable && (<div className={"data-table-content-header-expandable ".concat(allItemsExpanded ? "up" : "down")}>
                    <ButtonIcon size="md" buttonType="plain" typeIcon="keyboard_arrow_down" variant="primary" onClick={toggleExpandAllRows}/>
                  </div>)}
                {selectable && (<div className="data-table-content-header-checkbox" style={calculateLeftToCheckBox(expandable)}>
                    <InputCheckbox modelValue={selectAll} onUpdate={toggleSelectAll} indeterminate={isIndeterminate}/>
                  </div>)}

                {columns.map(function (column, columnIndex) { return (<div className={"th ".concat(columnIndex === 0 ? "sticky-first-column" : "")} style={calculateLeft(selectable, expandable)} key={columnIndex} onClick={function () { return handleSort(column); }}>
                    {column}
                    <div className="icon">
                      {sortConfig[column] === "asc" ? (<Icon icon="arrow_upward" size="sm"/>) : sortConfig[column] === "desc" ? (<Icon icon="arrow_downward" size="sm"/>) : (<Icon icon="swap_vert" size="sm"/>)}
                    </div>
                  </div>); })}
              </div>
              {filteredData
                .slice(indexOfFirstItem, indexOfLastItem)
                .map(function (row) { return (<div className="data-table-wrapper">
                    <div className="data-table-content-body" style={calculateGridTemplate(selectable, expandable)} key={row.id}>
                      {expandable && (<div className={"data-table-content-body-expandable ".concat(expandedRows.includes(row.id) ? "up" : "down")} key={row.id}>
                          <ButtonIcon size="md" buttonType="plain" typeIcon="keyboard_arrow_down" variant="primary" onClick={function () { return handleExpandRow(row.id); }}/>
                        </div>)}
                      {selectable && (<div className="data-table-content-body-checkbox" style={calculateLeftToCheckBox(expandable)} key={row.id}>
                          <InputCheckbox modelValue={selectedRows.includes(row.id)} onUpdate={function () { return toggleSelectRow(row.id); }}/>
                        </div>)}
                      {columns.map(function (_, columnIndex) { return (<div key={columnIndex} className={"fixed ".concat(columnIndex === 0 ? "sticky-first-column" : "")} style={calculateLeft(selectable, expandable)}>
                          <div key={row.id}>
                    ...
                    {expandedRows.includes(row.id) && expandedData && (<div className="data-table-content-expandable">
                        ...
                        .map(function (expandedItem) { return (<div key={expandedItem.id}>
                                {Object.keys(expandedItem)
                            .filter(function (key) { return key !== "id"; })
                            .map(function (key) { return (<div key={key}>{expandedItem[key]}</div>); })}
                              </div>); })}
                        </div>
                      </div>)}
                  </div>); })}
        ...
        <div className="data-table-footer">
          <Pagination label={label} variant="leftLabel" onClickRight={handleNextPage} onClickLeft={handlePrevPage}/>
        </div>
      </div>
    </>);
};
