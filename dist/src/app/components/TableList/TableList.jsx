    ...
    return (<div className={"table-list-root ".concat(isOverflowed ? "overflow-ativo" : "")} ref={tableListRef}>
      <div className="table">
        <div className="table-container">
          {columns.map(function (column, columnIndex) { return (<div className="th" key={columnIndex}>
              {column}
            </div>); })}
        </div>
        <div className="table-container">
          {transposedData.map(function (column, columnIndex) { return (<div key={columnIndex} className="table-column">
              {column.map(function (cell, cellIndex) { return (<div className={"td ".concat(size, " border")} key={cellIndex}>
                  {cell}
                </div>); })}
            </div>); })}
        </div>
