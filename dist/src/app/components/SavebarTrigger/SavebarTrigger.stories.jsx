...
var Template = function (args) {
    var _a, _b;
    var _c = useState(false), isOpenSidebar = _c[0], setIsOpenSidebar = _c[1];
    var _d = useState({
        ImageUploader: null,
        FileUploader: null,
    ...
    var _f = useState({}), initialValues = _f[0], setInitialValues = _f[1];
    var _g = useState(false), formChanged = _g[0], setFormChanged = _g[1];
    useEffect(function () {
        ...
        <Page namePage="Page Name" buttonContentSecondary="Button" withActionSecondary buttonContentPrimary="Button" withActionPrimary withBackButton>
          <SavebarTrigger labelCancel="Cancel" labelSave="Save" label="Unsaved changes" formChanged={formChanged} handleCancel={handleCancel} handleSubmit={handleSubmit}>
            <Layout columns="2 - Symmetric">
              <Input label="Input" placeholder="Placeholder" value={String(formValues["Input"])} onChange={function (e) {
            if (e.target.value !== null) {
                handleInputChange("Input", e.target.value);
            }
        }}/>
              <InputNumber label="Input Number" placeholder="Placeholder" min={0} max={10} value={String(formValues["Input Number"])} onChange={function (value) {
            handleInputChange("Input Number", value);
        }}/>
              <InputTime label="Input Time" placeholder="Placeholder" value={String(formValues["Input Time"])} onChange={function (value) {
            handleInputChange("Input Time", value);
        }}/>
              <InputSelect label="Input Select" value={String(formValues["Input Select"])} onChange={function (value) {
            handleInputChange("Input Select", value);
        }} options={options}/>
              <DataPicker label="Data Picker" placeholder="Placeholder" date={String(formValues["Input do DataPicker"])} onDateChange={function (value) {
            return handleDateChange("Input do DataPicker", value);
              ...
              <InputCheckbox label="Checkbox" onUpdate={function (checked) { return handleCheckboxChange(checked); }} modelValue={(_a = formValues["Checkbox"]) !== null && _a !== void 0 ? _a : false}/>

              <InputRadioButton label="Radio Button" onChange={function (checked) { return handleRadioChange(checked); }} checked={typeof formValues["Radio Button"] === "boolean"
            ? formValues["Radio Button"]
            : false}/>
              <Switch label="Switch" onUpdate={function (checked) { return handleSwitchChange(checked); }} modelValue={(_b = formValues["Switch"]) !== null && _b !== void 0 ? _b : false}/>
              <FileUploader buttonLabel="Add File" title="File Uploader" typeIconButton="upload_file" maxFileSize={10} multiple={true} value={formValuesFile["FileUploader"]} onChange={function (files) {
            return handleFileChange("FileUploader", files);
        }}/>
              <ImageUploader title="Image Uploader" proportion="1/1" labelDropzone={"Drop your images here"} iconDropzone={"add_a_photo"} multiple={true} value={formValuesFile["ImageUploader"]} onChange={function (files) {
            return handleFileChange("ImageUploader", files);
        }}/>
              <TextArea label="Text Area" placeholder="Placeholder" value={String(formValues["Text Area"])} onChange={function (value) { return handleInputChange("Text Area", value); }}/>
            </Layout>
          </SavebarTrigger>
        </Page>
      </AppShell>
