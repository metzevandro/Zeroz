  ...
  return <>
      {isClose && <div className={"notification ".concat(variant, " ").concat(type, " ").concat(dismissible && "dismissible", " ").concat(isOpen ? "open" : "")}>
          <div className={"notification-title ".concat(variant)}>
            <_Icon.default icon={icon} size="md" />
            <div className="title">{title}</div>
            {dismissible && <_ButtonIcon.default variant="on-color" size="sm" typeIcon="close" buttonType="plain" onClick={handleClickClose} />}
          </div>
          {showContent && <div className="notification-content">
              {description && <div className="description">{description}</div>}
              {withAction && <div className="notification-with-action">
                  <_Button.default size="md" variant={variant} disabled={disableButton} label={buttonLabel} onClick={onClickButton} />
                </div>}
            </div>}
        </div>}
    </>;
