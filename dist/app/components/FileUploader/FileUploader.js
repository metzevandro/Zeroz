...
var FileUploader = function FileUploader(_ref) {
  var title = _ref.title,
    ...
    typeIconButton = _ref.typeIconButton,
  ...
  (0, _react.useEffect)(function () {
    ...
    if (value && value.length > 0) {
      ...
      Array.from(value).forEach(function (file) {
        ...
        var errorMessage = hasError ? "O tamanho do arquivo \"".concat(file.name, "\" excede o limite de ").concat(maxFileSize, " MB.") : null;
        updatedFiles.push({
          file: file,
          hasError: hasError,
  ...
  var handleFileChange = /*#__PURE__*/function () {
    var _ref2 = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(event) {
      var files, updatedFiles, updatedFileList;
            ...
            if (files && files.length > 0) {
              ...
              if (multiple === true) {
                setSelectedFiles(function (prevFiles) {
                  return [].concat(_toConsumableArray(prevFiles), updatedFiles);
                });
              }

              // Crie um novo FileList usando DataTransfer
              updatedFileList = new DataTransfer();
              updatedFiles.forEach(function (fileObj) {
  ...
  var handleRemoveFile = function handleRemoveFile(index) {
    var updatedFiles = _toConsumableArray(selectedFiles);
    ...
    onChange(updatedFileListAsFileList.files.length > 0 ? updatedFileListAsFileList.files : null);
  };
  var _useState5 = (0, _react.useState)(null),
    _useState6 = _slicedToArray(_useState5, 2),
    tamanhoPai = _useState6[0],
    setTamanhoPai = _useState6[1];
  (0, _react.useEffect)(function () {
      ...
      <div className="file-uploader">
        <div className="file-uploader-header">
          ...
          <div className="file-uploader-button">
            <input disabled={disabled} type="file" ref={fileInputRef} multiple={!!multiple} style={{
            display: "none"
          }} onChange={handleFileChange} />
            <_Button.default variant="primary" disabled={disabled} onClick={handleButtonClick} label={buttonLabel} typeIcon={typeIconButton} size="md" />
          </div>
        </div>
        <div className="file-uploader-footer">
          ...
          {selectedFiles.length > 0 && <ul className="file-uploader-items">
            ...
            return <li className={"file-uploader-item ".concat(fileObj.hasError ? "file-uploader-item-error" : "")} key={index}>
                  <div className={"file-uploaded ".concat(fileObj.hasError ? "file-uploaded-error" : "")}>
                    ...
                    <p className="file-uploaded-icon">
                      {isLoading && <_Loading.default variant="default" />}
                      <div className="file-uploader-icon-error">
                        {fileObj.hasError && <_Icon.default icon="error_outline" size="sm" />}
                      </div>
                      <_ButtonIcon.default variant="primary" buttonType="plain" onClick={function () {
                    return handleRemoveFile(index);
                  }} size="sm" typeIcon="close" />
                    </p>
                  </div>
