...
function useInputNumber({ min, max, value, onChange, }) {
    ...
    const [numero, setNumero] = useState(() => resolveInitialValue(min));
...
const InputNumber = ({ max, min, placeholder, disabled = false, label, onChange, error = false, textError, value, }) => {
    ...
    return (jsxs("div", { className: "input-number-root", children: [label && (jsx("label", { className: "input-number-label", htmlFor: uid, children: label })), jsxs("div", { className: `input-number ${disabled ? "disabled" : ""}`, children: [jsx("button", { disabled: disabled, className: "subtract", onClick: decrement, children: jsx(Icon, { size: "md", icon: "remove" }) }), jsx("input", { id: uid, className: `input ${error ? "error" : ""}`, type: "number", inputMode: "numeric", placeholder: placeholder, value: numero !== undefined ? numero : "", max: max, min: min, disabled: disabled, onChange: handleInputChange, style: {
                            borderRight: isAddHover
                                ? "var(--s-border-width-hairline) solid var(--s-color-border-default-hover)"
                                : "",
                        } }), jsx("button", { disabled: disabled, className: "add", onClick: increment, onMouseEnter: () => setIsAddHover(true), onMouseLeave: () => setIsAddHover(false), children: jsx(Icon, { size: "md", icon: "add" }) })] }), error && jsx("div", { className: "textError", children: textError })] }));
};
