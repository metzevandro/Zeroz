...
function SelectDropdown({ options, selected, onSelect, isOpen, }) {
    return (jsxRuntime.jsx("ul", { className: `input-select-dropdown ${isOpen ? "open" : "close"}`, "aria-hidden": isOpen ? 'true' : 'false', role: "listbox", children: options.map((option) => (jsxRuntime.jsx("li", { role: "option", "aria-selected": option === selected, children: jsxRuntime.jsx("button", { className: `input-select-option ${option === selected ? "selected" : ""}`, onClick: () => onSelect(option), type: "button", children: option }) }, option))) }));
}
...
const InputSelect = ({ options, label, error = false, errorMessage, disabled = false, onChange, value, }) => {
    ...
    return (jsxRuntime.jsxs("div", { className: "input-select-root", ref: dropdownRef, children: [label && (jsxRuntime.jsx("label", { id: labelId, className: "input-select-label", htmlFor: uid, children: label })), jsxRuntime.jsxs("div", { className: "input-select", children: [jsxRuntime.jsxs("button", { id: uid, ref: triggerRef, type: "button", className: getTriggerClass(!!selectedOption, error, disabled), "aria-haspopup": "listbox", "aria-expanded": isOpen, "aria-labelledby": labelId, disabled: disabled || error, onClick: isOpen ? close : open, onKeyDown: handleKeyDown, children: [jsxRuntime.jsx("span", { className: selectedOption ? "" : "input-select-placeholder", children: selectedOption ?? label ?? "Select an option" }), jsxRuntime.jsx(Icon.Icon, { icon: "keyboard_arrow_down", size: "sm" })] }), jsxRuntime.jsx(SelectDropdown, { isOpen: isOpen, options: options, selected: selectedOption, onSelect: selectOption })] }), error && errorMessage && (jsxRuntime.jsx("p", { className: "input-select-error-message", children: errorMessage }))] }));
};
