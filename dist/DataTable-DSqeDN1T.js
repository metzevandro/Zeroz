 ...
 */
const DataTableRowHeader = ({ skeleton, headers, sortStates, columnWidths, withCheckbox, allSelected, someSelected, onSort, onSelectAll, }) => {
    return (jsxRuntime.jsxs("div", { style: { display: "flex", flex: "1" }, children: [withCheckbox && (jsxRuntime.jsx("div", { className: "data-table-body-header-checkbox", children: skeleton ? (jsxRuntime.jsx(Skeleton.Skeleton, { height: "24px", width: "24px" })) : (jsxRuntime.jsx(Checkbox.Checkbox, { indeterminate: someSelected, modelValue: allSelected, onUpdate: onSelectAll })) })), headers.map((header, index) => (jsxRuntime.jsx("div", { className: `data-table-row-header ${skeleton ? "loading-skeleton" : ""} ${index === 0 ? "first" : ""}`, style: { minWidth: columnWidths[index] }, onClick: () => onSort(index), children: skeleton ? (jsxRuntime.jsx(Skeleton.Skeleton, { height: "24px", width: "80px" })) : (jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [header, jsxRuntime.jsx(Icon.Icon, { icon: getSortIcon(sortStates[index]), size: "sm" })] })) }, header)))] }));
};
 ...
 */
const DataTableBody = ({ currentRows, selectedRows, skeleton, onRowSelection, columnWidths, withCheckbox, columnKeys, }) => {
    ...
    return (jsxRuntime.jsx("div", { className: "data-table-body-content", style: { flexDirection: "column" }, children: rows.map((row, index) => {
            ...
            return (jsxRuntime.jsx("div", { className: "data-table-body-content-row", children: jsxRuntime.jsxs("div", { style: { display: "flex", flex: "1" }, children: [withCheckbox && (jsxRuntime.jsx("div", { className: "data-table-body-content-checkbox", children: skeleton ? (jsxRuntime.jsx(Skeleton.Skeleton, { height: "24px", width: "24px" })) : (jsxRuntime.jsx(Checkbox.Checkbox, { disabled: skeleton, modelValue: selectedRows.includes(rowId), onUpdate: (checked) => onRowSelection(rowId, checked) })) })), columnKeys.map((key, colIndex) => (jsxRuntime.jsx("div", { className: "data-table-body-content-row", children: jsxRuntime.jsx(DataTableRowContent, { skeleton: skeleton, style: { minWidth: columnWidths[colIndex] }, children: row[key] }) }, key)))] }) }, rowId));
        }) }));
};
...
const DataTable = ({ columns, data, skeleton, rowsPerPage = 4, withCheckbox = false, headerSelectedChildren, textRowsSelected, onSelectedRowsChange, onUpdateSelectedRows, onSort, onSearch, page, totalItems, onPageChange, }) => {
    const { currentPage, currentRows, totalPages, loadedPages, isControlled, selectedRows, sortStates, allSelected, someSelected, handleSearch, handleSort, handleSelectAll, handleRowSelection, handlePageLeft, handlePageRight, } = useDataTable({
        data,
        rowsPerPage,
        columns,
        skeleton,
