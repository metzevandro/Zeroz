...
import { I as InputSearch } from './InputSearch-Cp_POJCm.js';
...
import { P as Pagination } from './Pagination-CoW_TpLg.js';
...
const DataTableHeader = ({ skeleton, onSearch, rowsSelected = 0, textRowsSelected, children, }) => {
    ...
    return (jsxs(Fragment, { children: [jsxs("div", { className: `data-table-header-rows-selected ${hasSelection ? "fade-in" : "fade-out"}`, style: {
                    visibility: hasSelection ? "visible" : "hidden",
                    display: hasSelection ? "" : "none",
                }, children: [jsx("p", { className: "textRowsSelected", style: { height: "40px", alignItems: "center", display: "flex" }, children: selectionLabel }, selectionLabel), jsx("div", { className: "data-table-header-rows-selected-buttons", children: children })] }), jsx("div", { className: `data-table-header ${hasSelection ? "fade-out" : "fade-in"}`, style: {
                    visibility: hasSelection ? "hidden" : "visible",
                    display: hasSelection ? "none" : "",
                }, children: jsx(InputSearch, { isSkeleton: skeleton, placeholder: "Procurar", onDebouncedChange: handleDebouncedChange, debounceMs: 0, onKeyDown: handleKeyDown }) })] }));
};
...
const DataTableRowHeader = ({ skeleton, headers, sortStates, columnWidths, withCheckbox, allSelected, someSelected, onSort, onSelectAll, sortable, }) => {
    return (jsxs("div", { style: { display: "flex", flex: "1" }, children: [withCheckbox && (jsx("div", { className: "data-table-body-header-checkbox", children: skeleton ? (jsx(Skeleton, { height: "24px", width: "24px" })) : (jsx(Checkbox, { indeterminate: someSelected, modelValue: allSelected, onUpdate: onSelectAll })) })), headers.map((header, index) => {
                const rowClasses = [
                    ...
                    .filter(Boolean)
                    .join(" ");
                return (jsx("div", { className: rowClasses, style: { minWidth: columnWidths[index] }, onClick: () => onSort(index), children: skeleton ? (jsx(Skeleton, { height: "24px", width: "80px" })) : (jsxs(Fragment, { children: [header, jsx(Icon, { icon: getSortIcon(sortStates[index]), size: "sm" })] })) }, header));
            })] }));
};
...
const DataTableRowContent = ({ children, skeleton, style, }) => {
    return (jsx("div", { className: "data-table-row-content", style: style, children: skeleton ? jsx(Skeleton, { height: "24px", width: "80px" }) : children }));
};
...
const DataTableBody = ({ currentRows, selectedRows, skeleton, onRowSelection, columnWidths, withCheckbox, columnKeys, rowHeight, onRowHeightChange, }) => {
    ...
    return (jsx("div", { className: "data-table-body-content", style: { flexDirection: "column" }, children: rows.map((row, index) => {
            const rowId = skeleton ? `skeleton-${index}` : row.id;
            return (jsx("div", { className: "data-table-body-content-row", ref: index === 0 ? firstRowRef : undefined, children: jsxs("div", { style: { display: "flex", flex: "1" }, children: [withCheckbox && (jsx("div", { className: "data-table-body-content-checkbox", children: skeleton ? (jsx(Skeleton, { height: "24px", width: "24px" })) : (jsx(Checkbox, { disabled: skeleton, modelValue: selectedRows.includes(rowId), onUpdate: (checked) => onRowSelection(rowId, checked) })) })), columnKeys.map((key, colIndex) => (jsx("div", { className: "data-table-body-content-row", children: jsx(DataTableRowContent, { skeleton: skeleton, style: { minWidth: columnWidths[colIndex] }, children: row[key] }) }, key)))] }) }, rowId));
        }) }));
};
...
const DataTableFooter = ({ currentPage, totalPages, disabledLeft, disabledRight, skeleton, onClickLeft, onClickRight, }) => {
    ...
    return (jsx("div", { className: "data-table-footer", children: jsx(Pagination, { label: label, variant: "leftLabel", onClickLeft: onClickLeft, onClickRight: onClickRight, disabledLeft: disabledLeft, disabledRight: disabledRight, skeleton: skeleton }) }));
};
...
function useDataTable({ data, rowsPerPage, columns, skeleton, onSelectedRowsChange, onUpdateSelectedRows, onSort, onSearch, page: controlledPage, totalItems, onPageChange, }) {
    ...
    const [sortStates, setSortStates] = useState(() => new Array(columns.length).fill("default"));
    const sortable = Boolean(onSort);
    const rowIdMap = useRef(new Map());
...
const DataTable = ({ columns, data, skeleton, rowsPerPage = 4, withCheckbox = false, headerSelectedChildren, textRowsSelected, onSelectedRowsChange, onUpdateSelectedRows, onSort, onSearch, page, totalItems, onPageChange, }) => {
    ...
    return (jsxs("div", { className: "data-table", children: [onSearch ? (jsx(DataTableHeader, { skeleton: isInitialLoading, onSearch: handleSearch, rowsSelected: selectedRows.length, textRowsSelected: textRowsSelected, children: headerSelectedChildren })) : null, jsxs("div", { ref: ref, className: `data-table-body ${isOverflowed ? "overflowed" : ""}`, style: {
                    height: rowsPerPage * rowHeight + 41.6,
                    opacity: isRefreshing ? 0.5 : 1,
                    pointerEvents: isRefreshing ? "none" : undefined,
                    transition: "opacity 0.15s ease",
                }, children: [jsx("div", { className: "data-table-body-header", children: jsx(DataTableRowHeader, { skeleton: isInitialLoading, headers: columnLabels, sortStates: sortStates, columnWidths: columnWidths, withCheckbox: withCheckbox, allSelected: allSelected, someSelected: someSelected, onSort: handleSort, onSelectAll: handleSelectAll, sortable: sortable }) }), isEmpty ? (jsx("div", { className: "data-table-body-empty", children: jsx(EmptyState, { title: "Nenhum resultado encontrado", description: "Tente ajustar os termos de pesquisa para encontrar o que procura.", icon: "search_off" }) })) : (jsx(DataTableBody, { withCheckbox: withCheckbox, columnWidths: columnWidths, columnKeys: columnKeys, currentRows: currentRows, selectedRows: selectedRows, skeleton: isInitialLoading, onRowSelection: handleRowSelection, rowHeight: rowHeight, onRowHeightChange: setRowHeight }))] }), jsx(DataTableFooter, { currentPage: currentPage, totalPages: totalPages, skeleton: isInitialLoading, onClickLeft: handlePageLeft, onClickRight: handlePageRight, disabledLeft: currentPage === 1 || currentRows.length === 0, disabledRight: isControlled
