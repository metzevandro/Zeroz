...
const DataTableHeader = ({ skeleton, onSearch, rowsSelected = 0, textRowsSelected, children, }) => {
    ...
    return (jsxs(Fragment, { children: [jsxs("div", { className: `data-table-header-rows-selected ${hasSelection ? "fade-in" : "fade-out"}`, style: {
                    ...
                    display: hasSelection ? "none" : "",
                }, children: jsx(InputSearch, { placeholder: "Procurar", disabled: skeleton, onDebouncedChange: handleDebouncedChange, debounceMs: 0, onKeyDown: handleKeyDown }) })] }));
};
...
const DataTableRowHeader = ({ skeleton, headers, sortStates, columnWidths, withCheckbox, allSelected, someSelected, onSort, onSelectAll, }) => {
    return (jsxs("div", { style: { display: "flex", flex: "1" }, children: [withCheckbox && (jsx("div", { className: "data-table-body-header-checkbox", children: skeleton ? (jsx(Skeleton, { height: "24", width: "24" })) : (jsx(Checkbox, { indeterminate: someSelected, modelValue: allSelected, onUpdate: onSelectAll })) })), headers.map((header, index) => (jsx("div", { className: `data-table-row-header ${skeleton ? "loading-skeleton" : ""} ${index === 0 ? "first" : ""}`, style: { minWidth: columnWidths[index] }, onClick: () => onSort(index), children: skeleton ? (jsx(Skeleton, { height: "24", width: "80" })) : (jsxs(Fragment, { children: [header, jsx(Icon, { icon: getSortIcon(sortStates[index]), size: "sm" })] })) }, header)))] }));
};
...
const DataTableRowContent = ({ children, skeleton, style, }) => {
    return (jsx("div", { className: "data-table-row-content", style: style, children: skeleton ? jsx(Skeleton, { height: "24", width: "80" }) : children }));
};
...
const DataTableBody = ({ currentRows, selectedRows, skeleton, onRowSelection, columnWidths, withCheckbox, columnKeys, }) => {
    ...
    return (jsx("div", { className: "data-table-body-content", style: { flexDirection: "column" }, children: rows.map((row) => {
            const rowId = row.id;
            return (jsx("div", { className: "data-table-body-content-row", children: jsxs("div", { style: { display: "flex", flex: "1" }, children: [withCheckbox && (jsx("div", { className: "data-table-body-content-checkbox", children: skeleton ? (jsx(Skeleton, { height: "24", width: "24" })) : (jsx(Checkbox, { disabled: skeleton, modelValue: selectedRows.includes(rowId), onUpdate: (checked) => onRowSelection(rowId, checked) })) })), columnKeys.map((key, index) => (jsx("div", { className: "data-table-body-content-row", children: jsx(DataTableRowContent, { skeleton: skeleton, style: { minWidth: columnWidths[index] }, children: row[key] }) }, key)))] }) }, rowId));
        }) }));
};
