...
const DataTableRowHeader = ({ skeleton, headers, sortStates, columnWidths, withCheckbox, allSelected, someSelected, onSort, onSelectAll, }) => {
    return (jsxs("div", { style: { display: "flex", flex: "1" }, children: [withCheckbox && (jsx("div", { className: "data-table-body-header-checkbox", children: skeleton ? (jsx(Skeleton, { height: "24px", width: "24px" })) : (jsx(Checkbox, { indeterminate: someSelected, modelValue: allSelected, onUpdate: onSelectAll })) })), headers.map((header, index) => (jsx("div", { className: `data-table-row-header ${skeleton ? "loading-skeleton" : ""} ${index === 0 ? "first" : ""}`, style: { minWidth: columnWidths[index] }, onClick: () => onSort(index), children: skeleton ? (jsx(Skeleton, { height: "24px", width: "80px" })) : (jsxs(Fragment, { children: [header, jsx(Icon, { icon: getSortIcon(sortStates[index]), size: "sm" })] })) }, header)))] }));
};
