...
function useInputSearch({ value, debounceMs, onChange, onDebouncedChange, }) {
    ...
    const debouncedEmit = useRef(debounce((val) => onDebouncedChange?.(val), debounceMs));
    useEffect(() => {
 ...
 * ```
 */
const InputSearch = ({ placeholder, disabled = false, onChange, onDebouncedChange, debounceMs = 300, value, isSkeleton, ...rest }) => {
    ...
    if (isSkeleton && dimensions.width > 0) {
        return (jsx(Skeleton, { width: `${dimensions.width}px`, height: `${dimensions.height}px` }));
    }
    return (jsx("div", { ref: containerRef, className: `input-search-root ${disabled ? "disabled" : ""}`, onClick: handleFocus, children: jsxs("div", { className: "input-search", children: [jsx(Icon, { size: "md", icon: "search" }), jsx("input", { ...rest, ref: inputRef, type: "text", size: 1000, placeholder: placeholder, value: inputValue, disabled: disabled, onChange: handleChange, onBlur: handleBlur }), inputValue && (jsx("button", { className: "input-search-button-close visible", onClick: handleClear, children: jsx(ButtonIcon, { variant: "primary", icon: "close", appearance: "plain", size: "sm" }) }))] }) }));
};
